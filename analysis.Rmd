---
title: "Pełna analiza Text Mining: newsy finansowe a indeks Dow Jones"
subtitle: "Przetwarzanie tekstu, częstość słów, TF-IDF oraz analiza sentymentu wieloma słownikami (Reproducible Research)"
author: "Kacper Tarnowski, Aleksander Mistur"
...
# Globalna konfiguracja chunkow. Ziarno losowosci -> pelna odtwarzalnosc.
knitr::opts_chunk$set(
  echo = TRUE, warning = FALSE, message = FALSE,
  fig.align = "center", fig.width = 9, fig.height = 5.5, dpi = 110,
  dev = "ragg_png"   # urzadzenie z obsluga UTF-8 -> polskie znaki na wykresach
)
set.seed(2026)
```
...
Celem projektu jest pełna analiza text mining oraz analiza sentymentu newsów
finansowych w zestawieniu z kierunkiem amerykańskiego indeksu **Dow Jones (DJIA)**.
System wczytuje dane z pliku CSV, w którym dla każdego dnia sesyjnego (2000–2016)
zebrano **25 najważniejszych nagłówków** oraz etykietę rynku (1 = DJIA wzrósł,
0 = spadł).

Skrypt łączy tradycyjne techniki przetwarzania tekstu (czyszczenie, tokenizacja,
**stemming** i uzupełnienie rdzeni — *stem completion*) z oceną sentymentu przy
użyciu wielu słowników: **Bing** (tidytext) oraz słowników z pakietu
`SentimentAnalysis` — **GI, HE, LM, QDAP**. Stosujemy trzy techniki poznane na
zajęciach: analizę częstości słów, TF-IDF oraz analizę sentymentu (z porównaniem
słowników i badaniem zmian w czasie).

# Wczytanie i przetwarzanie tekstu {#data}
...
# Technika 1 — Analiza częstości słów {#freq}
...
# Technika 2 — TF-IDF (dni wzrostowe vs spadkowe) {#tfidf}

Traktujemy nagłówki z dni wzrostowych i spadkowych jak dwa dokumenty i liczymy
TF-IDF, aby znaleźć słowa najbardziej charakterystyczne dla każdego kierunku rynku.
...
# Technika 3 — Analiza sentymentu {#sentiment}

## Słowa wnoszące sentyment (leksykon Bing)
...
```{r comparison-cloud, fig.width=8, fig.height=8}
tidy_news %>%
  inner_join(bing, by = c("term" = "word")) %>%
  count(term, sentiment, sort = TRUE) %>%
...
## Sentyment wieloma słownikami (GI, HE, LM, QDAP)

Łączymy 25 nagłówków danego dnia w jeden tekst i oceniamy go słownikami pakietu
`SentimentAnalysis`. Pozwala to **porównać wyniki różnych słowników**.
...
## Zmiana sentymentu w czasie (porównanie słowników)

```{r dict-time, fig.height=5.5}
scores %>%
  pivot_longer(c(GI, HE, LM, QDAP), names_to = "Dictionary", values_to = "value") %>%
...
## Sentyment a kierunek rynku

```{r lm-vs-market, fig.height=5}
...
# Interpretacja wyników i wnioski {#wnioski}

- **Częstość słów.** W nagłówkach z lat 2000–2016 dominuje słownictwo polityki
  międzynarodowej, konfliktów i gospodarki — typowe dla globalnych serwisów.
- **TF-IDF.** Zestawienie dni wzrostowych i spadkowych wskazuje słownictwo bardziej
  charakterystyczne dla każdego kierunku, choć różnice są subtelne.
- **Porównanie słowników.** Słowniki GI, HE, LM i QDAP dają zbliżony, ale nie
  identyczny obraz — różnią się czułością i udziałem dni neutralnych, co pokazuje,
  że wybór słownika wpływa na wynik analizy sentymentu.
- **Trend w czasie.** Wygładzone krzywe ujawniają okresy obniżonego sentymentu
  (m.in. wokół kryzysu 2008 r.) zgodnie dla różnych słowników.
- **Sentyment a rynek.** Porównanie dni wzrostowych i spadkowych oraz test t
  pokazują, jak słaby jest związek tonu nagłówków z dziennym kierunkiem DJIA —
  zgodnie z intuicją, że pojedynczy dzień newsów ma ograniczoną moc predykcyjną.

# Informacje o środowisku (odtwarzalność) {#sessioninfo}
